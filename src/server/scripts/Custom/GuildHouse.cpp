...
void SellGuildHouse(Player * pPlayer, Creature * pCreature)
{
        if(isInGuildArea(pPlayer))
        {
                ...
                pPlayer->TeleportTo(530, -1861.239990f, 5437.810059f, -10.463400f, 1.353830f);
        }
        else
        {
...
{
        if(pPlayer->GetMoney() >= (GOLD_COST * 1000))
        {
                ...
                pPlayer->SetMoney(pPlayer->GetMoney() - (GOLD_COST * 1000));
                requiresGUpdate = true;
        }
        else
        {
                pCreature->MonsterSay("You don't have enough gold!", LANG_UNIVERSAL, pPlayer->GetGUID());
        }
                pPlayer->PlayerTalkClass->SendCloseGossip();
}
